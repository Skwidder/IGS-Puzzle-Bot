...
async function migrateServer() {
  ...
  try {
    ...
    for (const doc of docs) {
      ...
      if (
        Array.isArray(doc.puzzle_queue) &&
        typeof doc.puzzle_queue[0] === "number"
      ) {
      ...
      if (doc.approved_collections) {
        for (let id of doc.approved_collections) {
          const response = await axios.get(
            `https://online-go.com/api/v1/puzzles/collections/${id}/`,
          );
      ...
      if (Object.keys(updates).length > 0) {
        if (DRY_RUN) {
          console.log(
            `[DRY RUN] Would update Server ${doc.name || doc._id}:`,
            updates,
          );
        } else {
          ...
          console.log(
            `Successfully migrated Server: ${doc.name || doc.serverId}`,
          );
        }
      }
    }
...
async function migratePlayer() {
  ...
  try {
    ...
    console.log(
      "Starting player migration: Nuking active_moves and initializing new fields...",
    );
    ...
    console.log(
      `Migration complete. Matched ${result.matchedCount} users and updated ${result.modifiedCount} documents.`,
    );
  } catch (error) {
