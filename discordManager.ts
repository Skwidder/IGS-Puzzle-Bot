...
export async function sendPuzzleSelectorMenu(
  user: User,
  inProgressServers: ServerConfig[],
) {
  ...
  if (inProgressServers.length > 1) {
    ...
    await user.send({
      content:
        "You have multiple in progress puzzles which would you like to do?",
      components: [row],
    });
  }
}

export async function sendAnnounceChannelMessage(
  client: IGSBot,
  serverId: string,
  text: string = "",
  embedPackage?: EmbedPackage,
  ...
  return await sendChannelMessage(
    client,
    channelId ?? server.announcementChannel,
    text,
    embedPackage,
  );
}
...
async function addAutoCompleteHandeler(
  interaction: AutocompleteInteraction,
): Promise<{ name: string; value: string }[] | null> {
  if (
    interaction.commandName !== "collection" &&
    interaction.commandName !== "puzzle"
  )
    return null;
  ...
  if (focusedOption.name === "search") {
    ...
    if (!provider) {
      return [
        {
          name: `Website: ${website} not found, Please select one from the list`,
          value: "none",
        },
      ];
    }
...
async function removeAutoCompleteHandler(
  interaction: AutocompleteInteraction,
): Promise<{ name: string; value: string }[] | null> {
