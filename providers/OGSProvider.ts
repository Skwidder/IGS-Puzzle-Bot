...
export class OGSProvider extends PuzzleProvider {
  ...
  async discoverPuzzles(
    ...
    do {
      response = await axios.get(
        `${this.apiBaseURL}?collection=${collectionSource.payload}&page_size=50&page=${i}`,
      );
  ...
  async getMoveResponse(
    ...
    if (!puzzle.initialPlayer)
      throw Error("OGS: Puzzle initial player is null");
    ...
    //Get response move
    const response =
      moveTree.branches[Math.floor(moveTree.branches.length * Math.random())];
  ...
  > {
    ...
    try {
      ...
      //without searching
      response = await axios.get(
        `https://online-go.com/api/v1/puzzles/collections/${searchString}`,
      );
    } catch (error) {
      return "ERROR";
    }
  ...
  async collectionAutocomplete(
    ...
    const response = await axios.get(
      `${this.apiBaseURL}collections?page_size=25&name__icontains=${name}`,
    );
