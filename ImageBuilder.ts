...
export class GoBoardImageBuilder {
  ...
  private calculateBoundingBox(padding = 1) {
    ...
    const minX = Math.min(
      ...this.stones.map((stone) => stone.x),
      ...this.marks.map((mark) => mark.x),
    );
    const maxX = Math.max(
      ...this.stones.map((stone) => stone.x),
      ...this.marks.map((mark) => mark.x),
    ...
    const minY = Math.min(
      ...this.stones.map((stone) => stone.y),
      ...this.marks.map((mark) => mark.y),
    );
    const maxY = Math.max(
      ...this.stones.map((stone) => stone.y),
      ...this.marks.map((mark) => mark.y),
    ...
    return {
      ...
      maxX: anchorLeft
        ? Math.min(this.size - 1, maxX + padding)
        : this.size - 1,
  ...
  private generateSVG(padding = 1): string {
    ...
    // Grid lines
    const topBleed =
      (fullHeight || box.anchorTop ? 0 : this.vBleed) * this.gridSize;
    const bottomBleed =
      (fullHeight || !box.anchorTop ? 0 : this.vBleed) * this.gridSize;
    const leftBleed =
      (fullWidth || box.anchorLeft ? 0 : this.hBleed) * this.gridSize;
    const rightBleed =
      (fullWidth || !box.anchorLeft ? 0 : this.hBleed) * this.gridSize;
    ...
    starPoints.forEach(([x, y]) => {
      if (x >= box.minX && x <= box.maxX && y >= box.minY && y <= box.maxY) {
        ...
        this.svgContent.push(
          `<circle cx="${px}" cy="${py}" r="${0.1 * this.gridSize}" fill="black"/>`,
        );
      }
    });
    ...
    this.stones.forEach(({ x, y, color }) => {
      ...
      // Stone shadow
      this.svgContent
        .push(`<circle cx="${px + 1}" cy="${py + 1}" r="${stoneRadius}" 
               fill="rgba(0,0,0,0.3)"/>`);
  ...
  private addCroppedCoordinateLabels(box: any) {
    ...
    for (let i = box.minX; i <= box.maxX; i++) {
      ...
      // Bottom labels
      this.svgContent
        .push(`<text x="${x}" y="${(box.maxY + 1) * this.gridSize + this.margin}" 
                text-anchor="middle" font-size="32">${letters[i]}</text>`);
    }
    ...
    for (let i = box.minY; i <= box.maxY; i++) {
      ...
      // Right labels
      this.svgContent
        .push(`<text x="${(box.maxX - box.minX + 1) * this.gridSize + this.margin - 10}" y="${y + 10}" 
