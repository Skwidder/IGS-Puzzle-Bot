...
async function execute(interaction: ChatInputCommandInteraction){
    ...
    switch(interaction.options.getSubcommand()){
        case "add":
            ...
            if(collectionResults === "COLLECTION_PRIVATE" || 
                collectionResults === "NO_COLLECTION_FOUND" ||
                collectionResults === "ERROR"
            ){
                return interactionReply(interaction,"Collection not found please check name!");
            } else if(collectionResults === "TOO_MANY_COLLECTIONS" ){
                return interactionReply(interaction, "More than one collection found, check name!");
            }

            const result = addCollection(client, interaction.guildId, collectionResults);
            if(!result) console.log(`[Add Collection] Collection found but add failed ${toAdd}`);

            interactionReply(interaction, `Collection ${collectionResults.name} from ${provider.name} added!`);
            break;
        case "remove":
            ...
            const removeResults = await removeCollection(client, interaction.guildId, source, collectionName);
