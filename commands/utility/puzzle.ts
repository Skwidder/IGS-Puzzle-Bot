...
const data = new SlashCommandBuilder()
  ...
  .addSubcommand((subcommand) =>
    subcommand
      .setName("next")
      .setDescription(
        "Move the server to next puzzle in the queue. This will remove the current puzzle",
      ),
  );
...
async function execute(interaction: ChatInputCommandInteraction) {
  ...
  switch (interaction.options.getSubcommand()) {
    ...
    case "remove":
      ...
      const removeResults = await removePuzzleFromQueue(
        client,
        interaction.guildId,
        item,
      );
    ...
    default:
      throw Error(
        `Puzzle subcommand not supported: ${interaction.options.getSubcommand()}`,
      );
      break;
  }
}
...
async function nextPuzzleHandle(interaction: ChatInputCommandInteraction) {
  ...
  const response = await advanceToNextPuzzle(
    interaction.client as IGSBot,
    interaction.guildId,
  );
  ...
  if (response.success === true) {
    interactionReply(
      interaction,
      "Moved to next puzzle, do /announce_puzzle to show it",
      undefined,
      true,
    );
    return;
  ...
  switch (response.errorType) {
    case "EMPTY_QUEUE":
      interactionReply(
        interaction,
        "No puzzles in queue using approved collection instead!",
      );
      return;
    case "COLLECTION_NOT_FOUND":
      interactionReply(
        interaction,
        "Collection not found it could have went private, check your collections",
      );
      return;
    case "NO_COLLECTIONS":
      interactionReply(
        interaction,
        "No puzzles in queue and no approved collections, please add a puzzle with" +
