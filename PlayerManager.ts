...
export async function userMessageHandle(message: Message) {
  if (message.content[0] == "!") {
    ...
    let activeServer = await getUserActiveServerState(
      client,
      message.author.id,
    );
    const inProgressServers: UserServerState[] | null =
      player?.guilds?.filter((g) => g.in_progress === 1) || null;

    if (!activeServer) {
      if (!inProgressServers || inProgressServers.length == 0) {
        sendUserDM(
          message.author,
          "You have no in-progress puzzles, please go on a server and do /play to add one",
        );
        return;
      } else if (inProgressServers.length == 1) {
        await setUserActiveServer(
          client,
          player.userId,
          inProgressServers[0].guildId,
        );
        activeServer = await getUserActiveServerState(
          client,
          message.author.id,
        );
      }
    ...
    const puzzle: ActivePuzzle | undefined = await getUserActivePuzzle(
      client,
      player,
    );
    ...
    if (message.content.toUpperCase() === "!RESET") {
      ...
      if (activeServer.active_moves.length > 1) {
        if (
          activeServer.active_moves[
            activeServer.active_moves.length - 1
          ].charAt(0) === responseColor
        ) {
      ...
      newMoveSGF = standardNotationToSGF(
        puzzle.initialPlayer,
      ...
      response = await puzzleProvider.getMoveResponse(
        puzzle,
        activeServer.active_moves,
        newMoveSGF,
      );
...
export async function interactionHandle(interaction: AnySelectMenuInteraction) {
  ...
  if (!user)
    throw Error(
      "[Player Manager] User responded to select string but is not a user?",
    );
  const puzzle = await getUserActivePuzzle(client, user);
  if (!puzzle) throw Error("[Player Manager] Race Condition");

  const activeServer: UserServerState | null =
    user?.guilds?.filter((g) => g.active === 1)[0] || null;
...
export async function playerPlay(interaction: ChatInputCommandInteraction) {
  ...
  let activeServer: UserServerState | null =
    player?.guilds?.filter((g) => g.active === 1)[0] || null;
  const inProgressServers: UserServerState[] | null =
    player?.guilds?.filter((g) => g.in_progress === 1) || null;
...
async function renderAndSendBoard(
  ...
  if (response?.responseMove)
    addUserMove(client, user.id, response.responseMove);
  ...
  const fields: APIEmbedField[] = infoToEmbedFields(
    client,
    puzzle,
    response,
    false,
...
async function getUserActivePuzzle(
  client: IGSBot,
  user: UserDocument,
): Promise<ActivePuzzle | undefined> {
  const activeServer: UserServerState | null =
    user?.guilds?.filter((g) => g.active === 1)[0] || null;
...
async function createUser(
  ...
  if (!result.acknowledged)
    throw Error(`[Player Manager]: User creation failed id ${userId}`);
